...
case class ItemTable(id: UUID, name: String, description: String, price: BigDecimal, categoryId: UUID, location: String, imageUrl: String)
case class ItemRepoPersist(ds: DataSource) extends ItemRepo {
  ...
  private val itemToTable = (item: Item) => ItemTable(item.id, item.name, item.description, item.price, item.categoryId, item.location, item.imageUrl)
  private val tableToItem = (itemTable: ItemTable) =>
    Item(itemTable.id, itemTable.name, itemTable.description, itemTable.price, itemTable.categoryId, itemTable.location, itemTable.imageUrl)
